...
const inviteCodes = [
'zyhNHbVVapsC4dVrAbu-ww==@gyOrv8CcnyW44TW0wKNIJg==@SF4tO5EDd0QlNtohFMKcQw==@u7wrmshTUPYSSy6_Mk9FpA==@_b9gGv6-64mJHWQsMbw4iA==@H6QeAqzrCwKDAqnhV5G_3g==@mQBtmnCNrQexrlpmZJMrng==@HoniDoRy0PZm8yJRz7ZrFg==',
'zyhNHbVVapsC4dVrAbu-ww==@gyOrv8CcnyW44TW0wKNIJg==@SF4tO5EDd0QlNtohFMKcQw==@u7wrmshTUPYSSy6_Mk9FpA==@_b9gGv6-64mJHWQsMbw4iA==@H6QeAqzrCwKDAqnhV5G_3g==@mQBtmnCNrQexrlpmZJMrng==@HoniDoRy0PZm8yJRz7ZrFg==',
'zyhNHbVVapsC4dVrAbu-ww==@gyOrv8CcnyW44TW0wKNIJg==@SF4tO5EDd0QlNtohFMKcQw==@u7wrmshTUPYSSy6_Mk9FpA==@_b9gGv6-64mJHWQsMbw4iA==@H6QeAqzrCwKDAqnhV5G_3g==@mQBtmnCNrQexrlpmZJMrng==@HoniDoRy0PZm8yJRz7ZrFg==',
'zyhNHbVVapsC4dVrAbu-ww==@gyOrv8CcnyW44TW0wKNIJg==@SF4tO5EDd0QlNtohFMKcQw==@u7wrmshTUPYSSy6_Mk9FpA==@_b9gGv6-64mJHWQsMbw4iA==@H6QeAqzrCwKDAqnhV5G_3g==@mQBtmnCNrQexrlpmZJMrng==@HoniDoRy0PZm8yJRz7ZrFg==',
];
...
!(async () => {
  ...
  await requireConfig();
  if (!cookiesArr[0]) {
    $.msg($.name, '【提示】请先获取京东账号一cookie\n选择【容器1】更新', 'http://ddns.ziyuand.cn:5555', {"open-url": "http://ddns.ziyuand.cn:5555"});
    return;
  }
  await requestAlgo();
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (!$.isLogin) {
        $.msg($.name, `【提示】cookie已失效`, `京东账号${$.index} ${$.nickName || $.UserName}\n请重新登录获取\nhttp://ddns.ziyuand.cn:5555`, {"open-url": "http://ddns.ziyuand.cn:5555"});
  ...
  //导出互助码
	...
	$.MyshareCodesArr = [];
	$.AllshareCodesArr = [];
	for (let s of $.tuanIds) {
	$.AllshareCodesArr.push($.tuanIds[s]);
	}
	$.MyshareCodesArr.push(
	{
		data: $.AllshareCodesArr,
		code: '200'
		}
	);
	console.log($.MyshareCodesArr)
	let str = JSON.stringify($.MyshareCodesArr, "", "\t");
	fs.writeFile('/ql/ninja/backend/static/code/dreamFactory_tuan.json',str,function(err){
		if (err) {
			console.log(`写文件失败`)
			}
			console.log(`写文件完成`)
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
			  ...
			  $.productskuId = data['commodityList'][0].skuId;
			  console.log(`---------------------------------`)
			  console.log(data['commodityList'][0].name);
			  console.log(data['commodityList'][0].skuId);
			  console.log(`---------------------------------`)
			  console.log(`产品名称：${$.productName} 产品ID: ${$.productskuId}`);
			  fanli($.productName,$.productskuId);
            } else {
              console.log(`GetCommodityDetails 异常：${JSON.stringify(data)}`)
            }
          }
        }
      } catch (e) {
            ...
            if (data['ret'] === 0) {
              ...
              const { shelvesList } = data;
			   //console.log(shelvesList);
			    for (let index = 0; index < shelvesList.length; index++) {
                    const { name,skuId } = shelvesList[index];
					//fanli(name,skuId);
                }
              if (shelvesList) {
	...
	$.get(
            ...
            (err, resp, data) => {
                try {
					console.log(data);
					console.log(resp);
				//console.log(`产品：${names} 访问成功：SKUID：${skuIds}`);
                } catch (e) {
                    $.logErr(e, resp);
                } finally {
                    resolve();
...
function taskflUrl(skuid) {
    return {
        url: `${JD_API_HOST}item/view?sku=${skuid}&_fd=jx&_fromappid=jwx2c49e82e87e57ff0&_jxgy=1&_ph_=1&ad_od=share&cu=true&utm_source=kong&utm_medium=jingfen&utm_campaign=t_1001528582_&utm_term=d34dac925edd46ad8356cad3ce02c9da&jxsid=16379777043284534487&sid_tkn=8912ddf4d8a1747bdfb7b0797f48dd47`,
        headers: {
...
function ChangeUrl(skuid) {
    return {
        url: `https://api.jd.com/routerjson?access_token=&app_key=f6bfc55e9b821f2ce9e342fdace1cf39&method=jd.union.open.promotion.common.get&v=1.0&sign=F2A9B49AA9FC3C096BD2CC111417F670&360buy_param_json=%7B%22promotionCodeReq%22%3A%7B%22siteId%22%3A%224100590443%22%2C%22materialId%22%3A%22https%3A%2F%2Fm.jingxi.com%2Fitem%2Fview%3Fsku%3D${skuid}%26_fd%3Djx%26_fromappid%3Djwx2c49e82e87e57ff0%26_jxgy%3D1%26_ph_%3D1%26ad_od%3Dshare%26utm_source%3Dandroidpingouapp%26utm_medium%3Dpingouappshare%26utm_campaign%3Dt_335139774%26utm_term%3DCopyURL%22%7D%7D&timestamp=2021-12-06+12%3A06%3A43.164%2B0800`,
        headers: {
            Cookie: $.currentCookie,
            Accept: `*/*`,
            Connection: `keep-alive`,
...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN('http://ddns.ziyuand.cn:5555/code/dreamFactory_tuan.json')
  if (!res) {
    $.http.get({url: 'http://21051.cn/code/dreamFactory_tuan.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await updateTuanIdsCDN('http://21051.cn/code/dreamFactory_tuan.json');
  }
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://ddns.ziyuand.cn:5555/code/dreamFactory.json`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function requireConfig() {
  return new Promise(async resolve => {
    ...
    // if (!tuanActiveId) {
    //   await updateTuanIdsCDN('http://ddns.ziyuand.cn:5555/code/dreamFactory_tuan.json');
    ...
    //       $.http.get({url: 'http://21051.cn/code/dreamFactory_tuan.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    //       await $.wait(1000)
    //       await updateTuanIdsCDN('http://21051.cn/code/dreamFactory_tuan.json');
    //       if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
    //         tuanActiveId = $.tuanConfigs['tuanActiveId'];
    //         console.log(`拼团活动ID: 获取成功 ${tuanActiveId}\n`)
    //       } else {
