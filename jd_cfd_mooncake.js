...
!(async () => {
  ...
  let res = await getAuthorShareCode('http://ddns.ziyuand.cn:5555/code/cfd.json')
  if (!res) {
    $.http.get({url: 'http://21051.cn/code/cfd.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('http://21051.cn/code/cfd.json')
  }
...
$.AllshareCodesArr = [];
for (let s = 0; s < 8; s++) {
$.AllshareCodesArr.push($.newShareCodes[s]);
}
...
fs.writeFile('/ql/ninja/backend/static/code/cfd.json',str,function(err){
	if (err) {
		console.log(`写文件失败`)
		}
		console.log(`写文件完成`)
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://ddns.ziyuand.cn:5555/code/cfd_mooncake.json`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
