...
!(async () => {
  ...
  let res = await getAuthorShareCode('http://ddns.ziyuand.cn:5555/code/jxhb.json')
  if (!res) {
    $.http.get({url: 'http://21051.cn/code/jxhb.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('http://21051.cn/code/jxhb.json')
  }
...
for(let i in $.packetIdArr){
	
	console.log($.packetIdArr[i]['strUserPin']);
	$.AllshareCodesArr.push($.packetIdArr[i]['strUserPin']);

}

$.MypacketIdArr.push(
{
	code: $.AllshareCodesArr,
	activeId: '525597'
	}
);
let str = JSON.stringify($.MypacketIdArr, "", "\t");
fs.writeFile('/ql/sharecode/jxhb.json',str,function(err){
	if (err) {
		console.log(`写文件失败`)
		}else{
		   	console.log(`写文件完成`) 
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    for (let j = 0; j < $.packetIdArr.length && $.canHelp; j++) {
            $.nowname=$.packetIdArr[j].userName;
     if($.nowname=='小炜king' || $.nowname=='jd_fFjKNIijtWOh' || $.nowname=='jd_bZcbcOOKwHCc' || $.nowname=='jd_hoQSfPaBjXVt' || $.nowname=='225584510-629559' || $.nowname=='c993265323' || $.nowname=='jd_atjGnAjwrHPI' || $.nowname=='jd_5027606ca8bdd' || $.nowname=='jd_SRohObGWOMOz'){
          ...
          if (data.iRet === 0) {
            ...
            if (data.Data.dwHelpedTimes === $.helpNum) {
              ...
              if (data.Data.strUserPin) {
                if($.UserName=='小炜king' || $.UserName=='jd_fFjKNIijtWOh' || $.UserName=='jd_bZcbcOOKwHCc' || $.UserName=='jd_hoQSfPaBjXVt' || $.UserName=='225584510-629559' || $.UserName=='c993265323' || $.UserName=='jd_atjGnAjwrHPI' || $.UserName=='jd_5027606ca8bdd' || $.UserName=='jd_SRohObGWOMOz'){
                $.packetIdArr.push({
