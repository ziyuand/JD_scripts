...
$.CASHBACKID = process.env.CASHBACKID !='' ? process.env.CASHBACKID.split('&') : [];
						
eval(common.eval.mainEval($));
async function prepare() {
	if($.CASHBACKID.length != '0'){
		for(let s in $.CASHBACKID){
			console.log('获取到设置的groupid：'+$.CASHBACKID[s]);
			let groupid = $.CASHBACKID[s];
			$.sharecode.push({
                 'groupid': groupid
                        })
		}
			...
			console.log('未获取到设置的groupid开始自动检测！');
		}
